                ...
                <div class="container-fluid">
                    <div class="header">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="dashboardAdmin.php">Manage Kiosk</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Kiosk    List</a></li>
                            </ol>
                        </nav>
                    </div>
                    <div class="col-12">
                            ...
                            <div class="card-body">
                                <table id="datatables-buttons" class="table table-striped" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <td class="text-xs">{{$kiosk->kioskNum}}</td>
                                        <td class="text-xs">{{$kiosk->kioskLocation}}</td>
                                        <td style="text-xs">
                                            @if ($kiosk->appStatus === "Available")
                                            <span class="badge rounded-pill bg-success">{{ $kiosk->appStatus}}</span>
                                            @else
                                            <span class="badge rounded-pill bg-warning ">{{ $kiosk->appStatus}}</span></td>
                                            @endif
                                        <td class="table-action">
                                            <a href="#" data-bs-toggle="modal" data-bs-target="#update-{{ $kiosk->id }}"><i class="align-middle fas fa-fw fa-pen"></i></i></a>
                                            <a href="/kiosk/{{$kiosk->id}}/delete"><i class="align-middle fas fa-fw fa-trash"></i></a>
                                        </td>
                                    </tr>
                                                    
                                <!-- Modal Kemaskini -->
                                <div class="modal fade" id="update-{{ $kiosk ->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body m-3">
                                                <form method="POST" action="/dashboard-admin/{{ $kiosk->id }}" enctype="multipart/form-data">
                                                    @csrf 
                                                    @method('PUT')
                                                    <input type="hidden" name="id" value="{{$kiosk->id}}">
                                                    <div class="form-group">
                                                        <label for="name">Kiosk Number</label>
                                                        <input type="number" class="form-control" id="kioskNum" name="kioskNum" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="kioskLocation">Kiosk Location</label>
                                                        <input type="text" class="form-control" id="kioskLocation" name="kioskLocation" required>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                                        <button type="submit" class="btn btn-info" name="addApp">Add</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
